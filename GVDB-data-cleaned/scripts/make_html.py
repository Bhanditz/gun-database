...
victimcolors = ['#64B5F6', '#2196F3', '#1976D2', '#0D47A1']
shootercolors = ['#E57373', '#F44336', '#D32F2F', '#B71C1C']
...
for row in csv.DictReader(open(sys.argv[1]), delimiter='\t'):
  i = int(row['Article id'])
  if True: #i < MAX:
    print "Processing article %s"%i
    url = row['Article url']
    title = row['Article title']
    raw_text = preproccess_text(row['Full text'])
    text = {}
    ...
    for role in ['shooter', 'victim']:
      for n,person in enumerate(data['%s-section'%role]):
	  ...
	  #print name == highlighted, '\t', '%s-%s'%(sidx, eidx), '\t', name, '\t', highlighted
          text = marktext(text, name, sidx, eidx, '%sname%s'%(role,(n/4)))
